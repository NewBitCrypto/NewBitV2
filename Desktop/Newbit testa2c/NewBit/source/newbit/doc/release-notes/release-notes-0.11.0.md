NewBit Core version 0.11.0 is now available from:

  <https://newbit.org/bin/newbit-core-0.11.0/>
...
defaults to 0.00001. This will cause transactions with fewer NBC/kB fee to be
...
For example, add the following to `newbit.conf`:
...
related to the blockchain in the newbit system: the raw blocks as received over 
the network (blk???.dat), the undo data (rev???.dat), the block index and the 
UTXO set (both LevelDB databases). The databases are built from the raw data.

Block pruning allows NewBit Core to delete the raw block and undo data once 
...
block index and UTXO databases. The minimum was chosen so that NewBit Core will 
...
For an example script see [newbit-submittx](https://github.com/laanwj/newbit-submittx).
...
- #5334 `cf87536` libnewbitconsensus: Add pkg-config support
...
- #6269 `95aca44` gitian: Use the new newbit-detached-sigs git repo for OSX signatures
...
- #5729 `9c4a5a5` Allow unit changes for read-only NewBitAmountField
- #5753 `0f44672` Add newbit logo to about screen
...
- #5809 `46bfbe7` Add newbit-cli man page
...
- #5849 `41259ca` contrib/init/newbitd.openrc: Compatibility with previous OpenRC init script variables
...
- #6116 `30dc3c1` [OSX] rename NewBit-Qt.app to NewBit-Core.app
- #6168 `b3024f0` contrib/linearize: Support linearization of testnet blocks
- #6098 `7708fcd` Update Windows resource files (and add one for newbit-tx)
