...
###   newbitcore.org and newbit.org and compares them.
...
WORKINGDIR="/tmp/newbit_verify_binaries"
TMPFILE="hashes.tmp"

SIGNATUREFILENAME="SHA256SUMS.asc"
RCSUBDIR="test"
HOST1="https://newbitcore.org"
HOST2="https://newbit.org"
BASEDIR="/bin/"
VERSIONPREFIX="newbit-core-"
RCVERSIONSTRING="rc"
...
if [ -n "$1" ]; then
   #let's also check if the version number includes the prefix 'newbit-',
...
if [ $? -ne 0 ]; then
   echo "newbit.org failed to provide signature file, but newbitcore.org did?"
   echo "wget output:"
   echo "$WGETOUT"|sed 's/^/\t/g'
...
if [ "$SIGFILEDIFFS" != "" ]; then
   echo "newbit.org and newbitcore.org signature files were not equal?"
...
if [ $RET -ne 0 ]; then
   if [ $RET -eq 1 ]; then
      ...
      echo "gpg error. Do you have the NewBit Core binary release signing key installed?"
   fi
