...
The RPM spec file provided here is for NewBit-Core 0.12.0 and builds on CentOS
...
reference the release tarball available from https://newbit.org/bin/ and
`Source1` should reference the BerkeleyDB source.

Sources 10-99 are for source files that are maintained in the
[NewBit git repository](https://github.com/newbit/newbit) but are not part of
...
Currently only `Source10` is used, for the example `newbit.conf` file.
...
in the newbit git repository. At present only an SVG version of the NewBit
icon is used.

## Patches

In general, patches should be avoided. When a packager feels a patch is
necessary, the packager should bring the problem to the attention of the newbit
...
needed when 0.12.1 is released, a proper fix is already in the NewBit git
master branch.
...
idea to build NewBit Core against a static version of that library compiled
...
The `newbit.svg` file is from the source listed as `Source100`. It is used as
...
matters for the `newbit-server` package. At this point, most RPM based
...
The `newbit-server` package should have SELinux support. How to properly do
...
NewBit-Core against OpenSSL or use a patched version of Boost in the build
system.
...
[Ringing Liberty](https://www.ringingliberty.com/newbit/). He has been
packaging NewBit for Fedora at least since 2012.
...
4. I renamed the `newbit` package that contains the Qt GUI to `newbit-core` as
that appears to be how the general population refers to it, in contrast to
`newbit-xt` or `newbit-classic`. I wanted to make sure the general population
