...
void CChainParams::UpdateForkPorts()
{
...
void CChainParams::UpdateForkPorts() const
{
    if (nDefaultPostForkPort && nDefaultPostForkPort != nDefaultPort && fork_conforksus.active) {
        (const_cast<CChainParams*>(this))->UpdateForkPorts();
    }
}
...
public:
    CMainParams()
    {
        ...
        vSeeds.emplace_back("seed.newbit.sipa.be", true); // Pieter Wuille, only supports x1, x5, x9, and xd
        vSeeds.emplace_back("dnsseed.bluematt.me", true); // Matt Corallo, only supports x9
        vSeeds.emplace_back("dnsseed.newbit.dashjr.org", false); // Luke Dashjr
        vSeeds.emplace_back("seed.newbitstats.com", true); // Christian Decker, supports x1 - xf
        vSeeds.emplace_back("seed.newbit.jonasschnelli.ch", true); // Jonas Schnelli, only supports x1, x5, x9, and xd
        ...
        checkpointData = (CCheckpointData){
            {
                ...
                // These can be re-enabled for fast initial sync, but ensure you do not add beyond the fork point
        ...
        chainTxData = ChainTxData{
            // If forking after height 478913, the 0,0,0 below can be replaced with the commented out section
            0,0,0
...
public:
    CTestNetParams()
    {
        ...
        vSeeds.emplace_back("testnet-seed.newbit.jonasschnelli.ch", true);
...
public:
    CRegTestParams()
    {
        ...
        consensus.BIP34Height = 500; // This is deactivated on newbit, but we activate it on the fork block here, as we otherwise won't get the fork hash coinbase
