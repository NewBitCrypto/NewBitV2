    ...
    if (role == Qt::DisplayRole || role == Qt::EditRole) {
        ...
        case Amount:
            if (rec->recipient.amount == 0 && role == Qt::DisplayRole)
                ...
                return NewBitUnits::format(walletModel->getOptionsModel()->getDisplayUnit(), rec->recipient.amount, false, NewBitUnits::separatorNever);
            else
                return NewBitUnits::format(walletModel->getOptionsModel()->getDisplayUnit(), rec->recipient.amount);
        }
...
{
    return (this->walletModel->getOptionsModel() != nullptr) ? tr("Requested") + " (" + NewBitUnits::name(this->walletModel->getOptionsModel()->getDisplayUnit()) + ")" : "";
}
