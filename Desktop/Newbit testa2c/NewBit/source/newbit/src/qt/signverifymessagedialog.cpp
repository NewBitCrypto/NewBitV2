...
SignVerifyMessageDialog::SignVerifyMessageDialog(const PlatformStyle* _platformStyle, QWidget* parent) : QDialog(parent),
...
void SignVerifyMessageDialog::on_signMessageButton_SM_clicked()
{
    ...
    CNewBitAddress addr(ui->addressIn_SM->text().toStdString());
    if (!addr.IsValid()) {
...
{
    CNewBitAddress addr(ui->addressIn_VM->text().toStdString());
    if (!addr.IsValid()) {
    ...
    if (!(CNewBitAddress(pubkey.GetID()) == addr)) {
        ui->statusLabel_VM->setStyleSheet("QLabel { color: red; }");
